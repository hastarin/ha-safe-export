...
**Decision:** `src/model.py` clamps the OLS point estimate to `MIN_CONSUMPTION_KWH = 1.0` for the heating and cooling zones (`est = max(est, MIN_CONSUMPTION_KWH)`). The empirical-table zones (warm_boundary, mild) are never clamped — a percentile of observed nights cannot be pathologically low the way a linear extrapolation can.
**Status:** Locked.
**Date:** 2026-07-04

**Rationale:** Both JS copies of the model (the Node-RED "Four-zone model" function node, and the now-deleted `tools/predictor.html`) applied `Math.max(1, baseConsumption)` to the OLS zones, but `src/model.py` had no equivalent — an undocumented divergence found in the 2026-07-03 audit. With the 2026-05-22 coefficients the clamp cannot bind for realistic inputs (heating minimum ≈5.6 kWh, cooling minimum ≈5.4 kWh, both well above 1.0 kWh) — it is currently dead code. The risk is post-retrain: nothing structurally prevents a future coefficient set from producing a near-zero or negative consumption estimate at an input extreme, which would silently inflate the safe-export recommendation. `1.0` kWh was chosen because it sits well below the observed minimum overnight consumption in the dataset — it only guards against pathological extrapolation, not normal model behaviour.

**Alternatives considered:** Removing the clamp from the JS copies for strict parity with the (then-unclamped) Python. Rejected because it would delete a safety net from the live system purely to satisfy a mirroring rule whose purpose is safety — CLAUDE.md's parity requirement exists to keep the live system _at least as safe_ as the canonical model, not to force the canonical model down to the live system's gaps.

**Operational note:** After any retrain (`tools/retrain.py`), check whether the floor would bind anywhere in the historical input range for the refit coefficients. If it does, the fit is suspect — investigate before deploying, don't just let the clamp mask it.
