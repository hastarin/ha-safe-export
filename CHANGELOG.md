...
- **CI now runs `mypy` over `src/` as a non-blocking reporting step (issue #41).** `[tool.mypy]` in `pyproject.toml` targets `src/` (`python_version = "3.11"`, `mypy_path = "."` so `from src.x import y` resolves when invoked from the repo root, `ignore_missing_imports = true` for `yaml`/`numpy`). Chosen over pyright/basedpyright because it's the more common default and needs no extra config to resolve the `src`-as-top-level-package layout. The CI step uses `continue-on-error: true` so it surfaces findings without failing the build; two genuine gaps it found were fixed rather than suppressed: `config.py`'s `_require` helper returned bare `object` (now `Any`, matching the untyped YAML data it actually reads), and `extract.py`'s metadata-ID lookups/`consumption_wh` computation needed narrowing from `int | None` to `int` at points already guaranteed non-`None` by `_get_metadata_ids`'s required-sensor check and the `missing`-fields early return (new `_required_id` helper, and a direct `is None` check replacing the dict-comprehension `missing` check for narrowing purposes). `mypy` reports clean over `src/`; `tools/` and `tests/` are out of scope for now. Flip to blocking once desired.
