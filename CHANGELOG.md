...
- **`Config.sensor_ids` and `src/extract.py`'s `_OPTIONAL_SENSORS` are now derived from `SensorConfig`'s dataclass fields instead of hand-maintained in three places (issue #39).** `sensor_ids` iterates `fields(self.sensors)`; `_OPTIONAL_SENSORS` is the set of fields whose dataclass default is `None` (which is exactly what "optional sensor" means here). Adding a sensor to `SensorConfig` now propagates to extraction automatically — previously forgetting it in `sensor_ids` meant a silently-NULL column, and forgetting it in `_OPTIONAL_SENSORS` meant a hard fail on installs lacking it. The derived set matches the previously-hardcoded `{guests, solcast, median_temp, median_humidity, forecast_temp, forecast_humidity}`; new tests in `tests/test_extract_synthetic.py` pin that contract.
- **`tools/backtest.py` and `tools/retrain.py` gained an `argparse` CLI matching `src/extract.py`'s conventions (issue #40).** Both accept `--dataset-db` (default `data/dataset.db`); `backtest.py` also accepts `--config` (default `config/config.yaml`). Defaults are identical to the previous hardcoded paths, so both tools still run with no arguments exactly as before; the module-level `DB_PATH`/`DEFAULT_*` constants are retained as the documented defaults for `BacktestParams`.
- **`tools/nodered-flow.json`'s "Four-zone model" function node now mirrors `src/model.py`'s temp-only fallbacks and fails closed instead of silently holding stale values (audit finding A1, issue #11 — this changes the live system).** Previously the node hard-validated all five inputs (`temp`, `humidity`, `solcast_kwh`, `soc`, `minSoc`) and returned `null` on any `NaN`, which meant `input_number.safe_export_wh` / `input_text.safe_export_detail` kept the *previous night's* values — nothing downstream could tell a failed prediction from a stale one. Now only `temp`, `soc`, `minSoc` are hard-required; missing `solcast_kwh` (heating zone) or `humidity` (cooling zone) falls back to the temp-only OLS variant (`H_TEMP_ONLY`/`C_TEMP_ONLY`, matching `heating_temp_only`/`cooling_temp_only` in `src/model.py`), noted in `model_info`. If a hard-required input is still `NaN`, the flow now writes `zone: "error"`, a `reason` string, a fresh timestamp, and `safe_export = 0` at every confidence level, so `automation.grid_export_start_at_scheduled_time`'s `target > 0` guard reliably sees 0 and skips the export session. `node.error()` is still called for the debug sidebar. New `tests/test_flow_parity.py` runs the extracted flow function under Node.js across a grid of (temp × soc × Solcast-present/absent × humidity-present/absent × confidence) and asserts parity with `predict()` within 1 Wh, plus dedicated fail-closed and fallback-variant checks; skips cleanly if `node` isn't on PATH. `tests/test_sync.py` extended to guard the new `H_TEMP_ONLY`/`C_TEMP_ONLY` constants and the renamed `MIN_CONSUMPTION_KWH` clamp constant against drift. **Requires a Node-RED re-import and a same-day manual verification (including simulating the failure path) before the next 18:00 run** — see `docs/analysis/LIVE_INTEGRATION.md`.
- **`src/model.py` now clamps the OLS consumption estimate to `MIN_CONSUMPTION_KWH = 1.0` kWh in the heating and cooling zones (audit finding A1 part 2, issue #10).** Both JS copies of the model (Node-RED's "Four-zone model" function node, and the now-deleted `tools/predictor.html`) already applied `Math.max(1, baseConsumption)`; the canonical Python had no equivalent, an undocumented divergence. Adopted as explicit policy on the Python side rather than removed from the JS — see `docs/DECISIONS.md` "Consumption-floor clamp on OLS zones". Non-binding on current (2026-05-22) coefficients — the clamp does not change tonight's export number — it exists to guard against a pathological post-retrain coefficient set extrapolating to a near-zero or negative consumption estimate. `tools/retrain.py` now reports whether the floor would bind anywhere in the historical input range after a refit. **Requires a Node-RED re-import to confirm the live flow's existing clamp constant stays in sync** (`tests/test_sync.py` now enforces this going forward).
- **`tools/backtest.py` no longer mutates module-level globals (audit finding A3/M3.1, issue #17).** `main()` previously set `BATTERY_WH`, `HARD_FLOOR_FRAC`, `SOFT_FLOOR_FRAC`, `BACKTEST_START`, `BACKTEST_END`, `EXPORT_RATE`, and `BUYBACK_RATE` via a `global` block, so importing any scenario/economics function without first running `main()` silently used the wrong defaults. Replaced with a frozen `BacktestParams` dataclass (`battery_wh`, `hard_floor_frac`, `soft_floor_margin`, `export_rate`, `buyback_rate`, `start`, `end`, `absence_periods`) built in `main()` via `BacktestParams.from_config()` from `Config` and the dataset's last date (deriving from the same `Config` that `predict()` reads, so the scoring capacity/floor cannot drift from what the model assumes), threaded explicitly through `accum_night`, `baseline_trough_soc`, the `run_*_scenario` functions, and the HTML/JSON builders. The module-level constants remain only as documented defaults for the dataclass constructor. `tests/test_backtest.py` now constructs `BacktestParams` explicitly instead of monkeypatching module state. Report output verified numerically identical before/after against the same dataset.
...
- **`tools/nodered-flow.json` now catches thrown errors from the five "Get …" state-reader nodes (and the model function node itself) instead of silently halting the flow (issue #36 — this changes the live system).** Previously a nonexistent entity ID (mistyped, renamed, or deleted) caused the `api-current-state` node to throw *before* reaching the model, leaving `input_number.safe_export_wh` / `input_text.safe_export_detail` holding the previous night's values — the fail-closed handling added for `unknown`/`unavailable` states (issue #11) did not cover this case. A new "Catch sensor read errors" `catch` node, scoped to the five reader nodes plus "Four-zone model", routes any thrown error to a new "Build fail-closed result" function node that produces the same zeroed shape (`zone: "error"`, `safe_export = 0` at every confidence, a `reason` string, a fresh `updated_at`) and feeds the existing writer path, so both HA helpers always get a fresh result instead of a stale one. `README.md`'s "Known issue" note is removed now that this is handled. **Requires a Node-RED re-import and a same-day manual verification (temporarily pointing a "Get …" node at a nonexistent entity, confirming both helpers zero out with a fresh timestamp, then restoring it) before the next 18:00 run** — see `docs/analysis/LIVE_INTEGRATION.md`.
- **`docs/analysis/LIVE_INTEGRATION.md` documents the sunrise / HA-start reset of `input_number.grid_export_target_wh` as the freshness backstop for the export chain (issue #37).** The automation (in the owner's HA config, not this repo) re-zeroes the copy target before each day's window, so a failed or stale Node-RED run cannot leave an export sized on old data in place for the next evening; only a fresh in-window copy repopulates it. This is why the copy automation (`grid_export_copy_forecast_to_target`, active only 17:55–18:05) does not need its own timestamp-freshness check. Doc-only; no code or live-flow change.
...
- **`tests/test_extract_synthetic.py`** — a portable, hand-computable synthetic HA recorder DB fixture (`statistics_meta`/`statistics` tables, matching the columns `src/extract.py`'s queries read) unit-testing the extraction primitives without the owner's personal HA database. Covers `_cum_delta` (normal delta, missing start/end bucket), `_forecast_at_6pm` (exact 18:00 hit, fallback within 3h, stale beyond 3h, sensor absent), `_get_metadata_ids` (required sensor missing raises `ValueError` naming it; optional sensor unconfigured or configured-but-absent both return `None`), `extract_row` (a complete synthetic day producing the expected dict, a day missing a required cumulative endpoint returning `None`), and the gap-warning heuristic in `extract_all` (an imbalanced day with near-zero battery throughput and a SOC swing triggers the warning log). Uses a purpose-built minimal `Config` with sensor names distinct from `tests/conftest.py`'s personal `test_cfg`, and `windows_for_date` for all timestamps so the fixture is timezone-correct by construction. Closes audit finding T2 (issue #15); the golden-fixture tests in `test_extract.py` are unchanged and remain the ground truth against the real HA database.
